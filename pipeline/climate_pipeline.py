# -*- coding: utf-8 -*-
"""Climate Security Index - Data Extraction Pipeline"""
import os
import json
import yaml
import requests
...
def generate_demo_data(config):
    ...
    return {
        "meta": {
            "project": "climate-security-index",
            "generated": datetime.now(timezone.utc).isoformat(),
            "sources": ["Copernicus Sentinel", "CHIRPS", "FEWS NET", "SPEI", "ACLED"],
            "version": "1.0.0"
        },
        "stats": [
            {"label": "Regions Scored", "value": "340", "delta": "+5%"},
            {"label": "Critical Risk", "value": "47", "delta": "+2"},
            {"label": "Climate Events", "value": "1,203", "delta": "-3%"},
            {"label": "Population Affected", "value": "89M", "delta": "stable"},
        ],
        "entities": [
        ...
        "events": [
            {"id": i, "title": f"Event {i}", "severity": ["low", "medium", "high"][i % 3], "timestamp": datetime.now(timezone.utc).isoformat(), "source": "Copernicus Sentinel"}
            for i in range(1, 16)
        ],
        "timeseries": [
